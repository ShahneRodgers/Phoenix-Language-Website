defmodule Language.TextModifier do
  ...
  defp create_update_function(word_map) when is_map(word_map) and map_size(word_map) > 0 do
    max_word_count =
      Enum.max_by(word_map, &calculate_original_word_count/1)
  ...
  defp search_for_match(word_map, word_parts, _max_word_count, i, _num_words)
       when length(word_parts) - 1 == i do
  ...
  defp search_for_match(word_map, word_parts, max_word_count, i, num_words)
       when length(word_parts) == i + num_words do
    ...
    if result do
      ...
      [Enum.at(word_parts, i) <> " "] ++
        search_for_match(word_map, word_parts, max_word_count, i + 1, 1)
    end
  end

  defp search_for_match(word_map, word_parts, max_word_count, i, num_words)
       when num_words == max_word_count do
    result = get_match_marked_up(word_map, word_parts, i, num_words)
  ...
  defp get_match_marked_up(word_map, word_parts, i, num_words) do
    str =
      Enum.slice(word_parts, i, num_words)
  ...
  defp create_additional_info_div(word, original) do
    ...
    nodes =
      nodes ++
        list_of_element_or_empty_if_nil(
          word.audio,
          {"audio", [{"src", word.audio}, {"class", "phoenix_translated_audio_link"}, "controls"],
           "Listen"}
        )
